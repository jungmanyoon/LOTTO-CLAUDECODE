...
def test_filter_manager_integration():
    ...
    try:
        ...
        if hasattr(filter_manager, 'performance_tracker') and filter_manager.performance_tracker:
            print("✅ 필터 매니저에 성능 추적기 통합 완료")
            
            # 실시간 통계 테스트
            stats = filter_manager.performance_tracker.get_real_time_stats()
            print(f"✅ 통합된 성능 추적기에서 통계 수집 가능")
            
        else:
            print("❌ 성능 추적기가 필터 매니저에 통합되지 않음")
            return False
        ...
        if hasattr(filter_manager.performance_tracker, 'get_filter_efficiency_scores'):
            ...
            print(f"✅ 필터 효율성 점수 계산 가능: {len(efficiency_scores)}개 필터")
        
        return True
        
    except Exception as e:
        print(f"❌ 필터 매니저 통합 테스트 실패: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_enhanced_dynamic_filter_manager():
    ...
    try:
        ...
        if report and 'filter_performances' in report:
            print(f"✅ 성능 보고서 생성 완료: {len(report['filter_performances'])}개 필터")
            
            # 실제 데이터 포함 여부 확인
            has_real_data = report.get('actual_filtering_data') is not None
            print(f"✅ 실제 필터링 데이터 포함: {'예' if has_real_data else '아니오'}")
            ...
            if non_zero_exclusions > 0:
                print(f"✅ {non_zero_exclusions}개 필터에서 실제 제외율 데이터 확인")
            else:
                print("⚠️ 모든 필터의 제외율이 0% - 실제 데이터 수집 필요")
        ...
        print(f"❌ 향상된 동적 필터 매니저 테스트 실패: {e}")
        import traceback
        traceback.print_exc()
        return False
