...
from src.core.optimization_checkpoint_manager import get_checkpoint_manager  # 체크포인트 관리자
from src.optimization.improved_auto_improvement_manager import get_improved_manager  # 개선된 자동 개선 관리자
    ...
    try:
        ...
        for i, pred in enumerate(all_ml_predictions):
            ...
            if len(numbers) == 6:
                ...
                # 완화 레벨 결정 (3단계)
                # ✓ 수정: 0.7 → 0.6 (Level 3 접근성 향상)
                if prediction_confidence > 0.6 or model_name in ['ensemble', 'combined', 'lstm']:
                    relaxation_level = 3  # 최고 완화
                elif prediction_confidence > 0.5 or need_more_relaxation:
                ...
                if passes_filter:
                    ...
                    final_predictions.append({
                        ...
                        'source': f"ML-Enhanced/{model_name} (L{relaxation_level})",
                        'filter_info': filter_result
                    })
                    ml_inclusion_stats['passed'] += 1
                    logging.info(f"ML 예측 통과 [{i+1}/{len(all_ml_predictions)}]: {sorted(numbers)} "
            ...
            if not filtered_combos:
                ...
                for failed_pred in ml_failed_predictions[:10]:  # 최대 10개 시도
                    ...
                    if not recovery_attempted:
                        ...
                        if similar_combos and similar_combos[0]['similarity'] > 0.5:  # 유사도 임계값 상향
        ...
        if len(final_predictions) < num_sets:
            ...
            try:
                if not 'filtered_combos' in locals():
                    # 필터링된 조합 가져오기 - 최신 회차 자동 감지
                    target_round = db_manager.get_last_round()
                    filtered_combos = db_manager.combinations_db.get_filtered_combinations(target_round) or []
                ...
                if filtered_combos:
                    # 필터링 풀에서 랜덤하게 선택하되, 중복 방지
                    import random
                    random.seed(42)  # 재현 가능한 결과를 위한 시드 설정
                    available_combos = []

                    for combo_str in filtered_combos[:1000]:  # 상위 1000개에서 선택
                        try:
                            numbers = [int(x) for x in combo_str.split(',')]
                            if not _is_duplicate_prediction(numbers, final_predictions):
                                available_combos.append(numbers)
                        except (ValueError, IndexError):
                            continue

                    # 부족한 만큼 추가
                    needed = num_sets - len(final_predictions)
                    selected_combos = random.sample(available_combos, min(needed, len(available_combos)))

                    for numbers in selected_combos:
                        final_predictions.append({
                            'numbers': sorted(numbers),
                            'confidence': 0.3,  # 기본 신뢰도
                            'source': "Pool-Supplement"
                        })

                    logging.info(f"[풀 보충] {len(selected_combos)}개 조합 추가")
            except Exception as e:
        ...
        if final_inclusion_rate >= target_inclusion_rate:
            ...
            logging.info(f"  - ⚠️ 목표 미달성 (차이: {(target_inclusion_rate - final_inclusion_rate):.1%})")
...
def parse_args():
    ...
    if len(sys.argv) == 1:
        ...
        args.dashboard = True  # ✅ 대시보드도 자동 활성화
...
def main():
    ...
    global smart_learning_global, continuous_improvement_global, optimization_thread_global, optimization_stop_flag
    smart_learning_global = None
    continuous_improvement_global = None
    optimization_thread_global = None
    ...
    def graceful_shutdown(signum=None, frame=None):
        ...
        try:
            ...
            # 백그라운드 최적화 스레드 종료
            if optimization_thread_global and optimization_thread_global.is_alive():
                logging.info("[BACKGROUND OPTIMIZATION] 백그라운드 최적화 스레드 종료 중...")
                optimization_stop_flag['stop'] = True
                optimization_thread_global.join(timeout=5)  # 최대 5초 대기
                if optimization_thread_global.is_alive():
                    logging.warning("[BACKGROUND OPTIMIZATION] 스레드가 5초 내에 종료되지 않음 (강제 종료됨)")
                else:
                    logging.info("[BACKGROUND OPTIMIZATION] 스레드 정상 종료 완료")
    ...
    # ========================================
    # 최적화 체크포인트 시스템 처리 (DEPRECATED)
    # ========================================
    # NOTE: 이제 백그라운드 스레드로 자동 실행됩니다.
    #       별도의 --optimize-only 실행은 불필요합니다.
    if args.optimize_threshold or args.continue_optimization:
        logging.warning("\n" + "="*60)
        logging.warning("⚠️ --optimize-threshold 플래그는 더 이상 필요하지 않습니다.")
        logging.warning("   백그라운드 최적화가 main.py 실행 시 자동으로 시작됩니다.")
        logging.warning("   이 모드는 곧 제거될 예정입니다.")
        logging.warning("="*60 + "\n")

        # 기존 코드는 유지하되, 곧 deprecated될 것임을 알림
        logging.info("\n" + "="*60)
        logging.info("📊 임계값 최적화 시스템 시작 (호환성 모드)")
        logging.info("="*60)
        ...
        try:
            ...
            if args.reset_checkpoint:
                ...
                checkpoint_manager.reset_optimization_progress()
                logging.info("✅ 최적화 체크포인트가 초기화되었습니다.")

            # 최적화 실행
            optimizer = AutoThresholdOptimizer()

            # 중단된 최적화 이어서 실행 또는 새로 시작
            iteration_count = 0
            cycle_count = 0  # 140개 조합 사이클 카운터
            max_iterations = args.optimization_iterations if args.optimization_iterations > 0 else float('inf')

            # 무한 반복 모드 설정 (optimization_iterations가 0이면 무한 반복)
            infinite_mode = (args.optimization_iterations == 0)

            if infinite_mode:
                logging.info("⚡ 무한 반복 백그라운드 최적화 모드 활성화")
                logging.info("   - 140개 조합 완료 후 자동 재시작")
                logging.info("   - 누적 학습: 0→140→280→420...")

            while iteration_count < max_iterations:
                iteration_count += 1

                if args.optimization_iterations > 0:
                    logging.info(f"\n🔄 최적화 반복 {iteration_count}/{args.optimization_iterations}")
                else:
                    logging.info(f"\n🔄 최적화 반복 {iteration_count} (무제한 모드, 사이클 #{cycle_count + 1})")

                result = optimizer.optimize_with_checkpoint(infinite_mode=infinite_mode)

                if result['status'] == 'completed':
                    logging.info("✅ 모든 최적화 조합 테스트 완료!")
                    if result.get('best_params'):
                        logging.info(f"최적 파라미터: 임계값={result['best_params']['threshold']}%, "
                                   f"ML바이패스={result['best_params']['ml_bypass']}, "
                                   f"ML가중치={result['best_params']['ml_weight']}")
                    # 무한 모드가 아닐 때만 종료
                    if not infinite_mode:
                        break
                elif result['status'] == 'cycle_completed':
                    # 무한 모드에서 한 사이클 완료
                    cycle_count += 1
                    logging.info(f"♻️ 사이클 #{cycle_count} 완료 (총 테스트: {cycle_count * 140}개)")
                    if result.get('best_params'):
                        logging.info(f"📊 현재까지 최적 파라미터: 임계값={result['best_params']['threshold']}%, "
                                   f"ML바이패스={result['best_params']['ml_bypass']}, "
                                   f"ML가중치={result['best_params']['ml_weight']} "
                    ...
                    logging.error(f"최적화 에러: {result.get('error')}")
                    break

                # 진행상황 표시
                if result['status'] == 'in_progress' and 'progress' in result:
                    progress = result['progress']
                    total_tests = cycle_count * 140 + progress['completed_combinations']
                    logging.info(f"📊 진행률: {progress['completed_combinations']}/{progress['total_combinations']} "
                                f"({progress['progress_percentage']:.1f}%) | 총 누적: {total_tests}개 테스트")

            # 개선된 자동 개선 관리자와 통합
            improved_manager = get_improved_manager()
            summary = improved_manager.get_optimization_summary()
            logging.info(f"\n📈 최적화 통계:")
            logging.info(f"  - 총 백테스팅: {summary['total_backtests']}회")
            logging.info(f"  - 개선 횟수: {summary['improvements_count']}회")
            logging.info(f"  - 현재 임계값: {summary['current_threshold']:.1f}%")

            logging.info("\n최적화 완료. 메인 프로그램 계속 실행...")
            logging.info("="*60 + "\n")

        except Exception as e:
            logging.error(f"최적화 시스템 실행 실패: {e}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if state_mgr.check_sync_needed(latest_round):
                    ...
                    logging.info("[시작 동기화] ✅ 자동 업데이트 완료")
                else:
                    logging.info("[시작 동기화] ✅ 시스템 동기화 상태 양호")
            ...
            # ========================================
            # [NEW] 필터 사전 검증 및 자동 조정 시스템
            # ========================================
            try:
                logging.info("\n[사전 검증] 필터 시스템 검증을 시작합니다...")
                from src.validators.filter_validator import FilterValidator as ValidatorFilterValidator
                from src.core.filter_manager import FilterManager
                ...
                # 필터 검증 시스템 초기화
                filter_validator = ValidatorFilterValidator(db_manager)

                # 최근 50회차로 빠른 검증 실행 (더 최근 패턴에 집중)
                recent_validation = filter_validator.validate_filters_with_historical_data(
                    start_round=max(1, db_manager.get_last_round() - 49),  # ✅ FIX: 100→50 (일관성 향상)
                    end_round=db_manager.get_last_round(),
                    is_startup=True  # 시작 시 검증임을 표시 (데이터 준비 확인)
                )

                # 검증 결과 확인 및 자동 조정
                if recent_validation.get('overall_pass_rate', 100) < 85:
                    logging.warning(f"[사전 검증] 필터 통과율이 {recent_validation['overall_pass_rate']:.1f}%로 낮아 자동 조정을 실행합니다.")
                else:
                    logging.info(f"[사전 검증] 필터 통과율 {recent_validation['overall_pass_rate']:.1f}% - 정상 범위입니다.")

            except Exception as e:
                logging.warning(f"[사전 검증] 필터 검증 중 오류 발생: {e}")
                logging.info("[사전 검증] 프로그램을 계속 진행합니다...")
            ...
            try:
                ...
                # 초기 백테스팅 실행 (옵션)
                # NOTE: 구버전 Grid Search는 비활성화 - Optuna TPE 기반 최적화 사용
                if not args.skip_backtest:
                    logging.info("[CONTINUOUS IMPROVEMENT] 백그라운드 최적화 시스템 활성화됨")
                    logging.info("   - [O] Optuna TPE 기반 Bayesian 최적화 사용")
                    ...
                    #     logging.info(f"✅ 초기 최적화 완료: 성능 {optimization_result.get('improvement_rate', 0):.1%} 개선")
                    # else:
            ...
            # ========================================
            # [AUTO-OPT] 백그라운드 최적화 자동 시작
            # ========================================
            # F5 실행 시 모든 것이 자동으로 실행되도록 통합
            try:
                import threading
                from src.scripts.auto_threshold_optimizer import AutoThresholdOptimizer

                def background_optimization_worker():
                    """백그라운드에서 무한 반복 최적화를 수행하는 워커 스레드 (Optuna TPE 기반)"""
                    try:
                        logging.info("⚡ 백그라운드 최적화 스레드 시작")
                        logging.info("   - 🔬 Optuna TPE 기반 Bayesian 최적화")
                        logging.info("   - 무한 반복 모드 (10 trials per cycle)")
                        logging.info("   - 누적 학습: 0→10→20→30... (지능적 파라미터 탐색)")
                        logging.info("   - SQLite 기반 persistence (자동 중단/재시작)")
                        logging.info("   - 🎯 수렴 감지: patience=50, threshold=1%, max_trials=500")

                        optimizer = AutoThresholdOptimizer()
                        cycle_count = 0
                        converged = False

                        while not optimization_stop_flag['stop'] and not converged:  # 종료 플래그 및 수렴 확인
                            try:
                                # Optuna TPE 기반 최적화 실행 (Grid Search 대체)
                                ...
                                if result.get('converged', False):
                                    ...
                                    logging.info(f"[CONVERGED] [백그라운드] 최적화 수렴 감지: {convergence_reason}")
                                    logging.info(f"   최적 파라미터: threshold={result['best_params']['threshold']:.2f}%, "
                                               f"ml_bypass={result['best_params']['ml_bypass']}, "
                                               f"ml_weight={result['best_params']['ml_weight']:.2f}")
                                    logging.info(f"   최적 점수: {result['best_score']:.4f}")
                                    logging.info(f"   총 누적 trials: {result['total_trials']}개")
                                    break

                                if result['status'] == 'cycle_completed':
                                    cycle_count += 1
                                    total_trials = result.get('total_trials', cycle_count * 10)
                                    logging.info(f"♻️ [백그라운드] 사이클 #{cycle_count} 완료 (총 누적 trials: {total_trials}개)")
                                    if result.get('best_params'):
                                        logging.info(f"📊 [백그라운드] 현재 최적: 임계값={result['best_params']['threshold']}%, "
                                                   f"ML바이패스={result['best_params']['ml_bypass']}, "
                                                   f"ML가중치={result['best_params']['ml_weight']} "
                                                   f"(점수: {result['best_score']:.3f})")
                                    continue
                                elif result['status'] == 'completed':
                                    logging.info(f"✅ [백그라운드] 최적화 정상 완료 (총 trials: {result.get('total_trials')})")
                                    break
                                elif result['status'] == 'error':
                                    logging.error(f"[백그라운드] 최적화 에러: {result.get('error')}")
                                    break

                            except KeyboardInterrupt:
                                logging.info("[백그라운드] 최적화 중단 요청")
                                break
                            except Exception as e:
                                logging.error(f"[백그라운드] 최적화 오류: {e}")
                                import time
                                time.sleep(60)  # 1분 대기 후 재시도

                        if converged:
                            logging.info("[CONVERGED] [백그라운드] 최적화 수렴으로 종료 (최적 파라미터 적용됨)")
                        logging.info("⚡ 백그라운드 최적화 스레드 종료")
                    except Exception as e:
                        logging.error(f"[백그라운드] 최적화 워커 초기화 실패: {e}")

                # 백그라운드 최적화 스레드 시작
                optimization_thread_global = threading.Thread(
                    target=background_optimization_worker,
                    daemon=True,  # 메인 프로그램 종료 시 자동 종료
                    name="BackgroundOptimization"
                )
                optimization_thread_global.start()
                logging.info("[AUTO-OPT] ✅ 백그라운드 최적화 자동 시작됨")

            except Exception as e:
                logging.warning(f"[AUTO-OPT] 백그라운드 최적화 시작 실패 (계속 진행): {e}")
        ...
        #     logging.info("✅ 메모리 효율성 패치 및 관리자 적용됨")
        ...
        if args.stream_processing:
            logging.info("\n[🚀 스트림 처리 모드] 메모리 효율적인 스트림 기반 필터링을 사용합니다.")
                        ...
                        if not ensemble_predictor.is_trained:
                            ...
                            try:
                                ...
                                if "least populated class" in error_msg or "minimum number of groups" in error_msg:
                                    ...
                                    try:
                                        # 대체 모델 사용 시도
                                        from src.ml.ensemble_predictor_fixed import EnsemblePredictor as FixedEnsemblePredictor
                                        ensemble_predictor = FixedEnsemblePredictor()
                                        evaluation = ensemble_predictor.train(winning_numbers, test_size=0.3)
                                        logging.info("  - 대체 앙상블 모델 학습 성공")
                                    except Exception as fallback_e:
                                        logging.error(f"  - 대체 모델도 실패: {fallback_e}")
            ...
            if filtered_count == 0:
                ...
                try:
                    ...
                    if filtered_count_after > 0:
                        logging.info(f"[✓] 필터링 재실행 성공: {filtered_count_after:,}개 조합 생성")
                    else:
                        logging.error(f"[✗] 필터링 재실행 실패: 여전히 0개 조합")
                        logging.info("  - 백테스팅은 이전 회차 데이터를 사용합니다")

                except Exception as regen_error:
                    logging.error(f"[✗] 필터링 재실행 중 오류: {regen_error}")
                    logging.info("  - 백테스팅은 이전 회차 데이터를 사용합니다")
            else:
                logging.info(f"[✓] 필터링된 조합 풀 정상: {filtered_count:,}개")
            # ============================================================
        ...
        # ================================================================
        # 대시보드 실행 (--dashboard 플래그가 있을 때만, opt-in 방식)
        # ================================================================
        if args.dashboard:
            try:
                logging.info("\n" + "="*60)
                ...
                def run_dashboard():
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.error("대시보드 모듈을 찾을 수 없습니다.")
                    except Exception as e:
...
def _apply_hybrid_filtering(numbers, filter_manager, db_manager, relaxation_level, ml_threshold=None):
    ...
    try:
        ...
        # AdaptiveProbabilityFilter 처리
        if hasattr(filter_manager, 'probability_threshold'):
            # ML 전용 임계값으로 임시 변경
            # Fallback: ThresholdManager에서 기본값 가져오기 (BUG-002 수정)
            from src.core.threshold_manager import get_threshold_manager
            threshold_manager = get_threshold_manager()
            ...
            try:
                ...
                filtered_result = filter_manager.apply([numbers_str], db_manager.get_last_round())

                if filtered_result:  # 통과
                    return True, {
...
def _find_enhanced_similar_combinations(target_numbers, filtered_combos, top_n=10):
    ...
    for combo_str in filtered_combos[:10000]:  # 성능을 위해 최대 10K개만 검사
        try:
