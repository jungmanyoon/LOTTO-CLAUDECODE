...
def main():
    ...
    try:
        ...
        # ================================================================
        # [Phase 3] UnifiedOptimizer 백그라운드 최적화 시작 (필터링 완료 후)
        # ================================================================
        # 필터링이 CPU/DB를 집중 사용하므로 완료 후에 Optuna 시작
        if unified_optimizer_global is not None:
            try:
                unified_optimizer_global.start_background(optimization_stop_flag)
                logging.info("[UnifiedOptimizer] 백그라운드 최적화 시작 (필터링 완료 후)")
            except Exception as e:
            ...
            try:
                ...
                def run_dashboard():
                    ...
                    try:
                        ...
                        logging.error("대시보드 v2 모듈을 찾을 수 없습니다.")
                    except Exception as e:
