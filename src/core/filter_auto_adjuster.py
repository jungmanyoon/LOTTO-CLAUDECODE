...
class FilterAutoAdjuster:
    ...
    def __init__(self, db_manager, filter_manager):
        ...
        # [O] NEW: PerformanceTracker 인스턴스 (필터 스냅샷 저장용)
        try:
    ...
    def apply_optimized_criteria(self, optimized_criteria: Dict, validation_results) -> bool:
        ...
        logging.info("\n" + "="*60)
        logging.info("[FIX] 필터 자동 조정 시스템 작동")
        logging.info("="*60)
        logging.info("[LIST] 조정 대상 분석 중...")
        ...
        if isinstance(validation_results, dict):
            ...
            if overall_rate < 85:
                ...
                logging.warning(f"   [WARN] 주의: 통과율이 90% 미만입니다!")
        ...
        for filter_name, filter_result in validation_results.get('filter_results', {}).items():
            ...
            status_icon = "[O]" if pass_rate >= 90 else "[WARN]" if pass_rate >= 85 else "[!!]"
        ...
        # [HOT] FIX: 전체 통과율이 85% 미만이면 개별 통과율 85% 이상이라도 가장 낮은 3개 필터 조정
        filters_to_adjust = []
        ...
        if filters_to_adjust:
            logging.info(f"\n[TARGET] 조정 대상 필터 ({len(filters_to_adjust)}개):")
            for filter_name, pass_rate in filters_to_adjust:
                logging.info(f"   - {filter_name}: {pass_rate:.1f}%")

        # [FIX N-W10] 멀티 필터 조정: 누적 config를 사용하여 이전 변경사항 보존
        # 루프 시작 전에 YAML을 한 번만 로드, 이후 _adjust_filter()가 이 캐시를 재사용
        try:
            with open(self.config_path, 'r', encoding='utf-8') as f:
                self._temp_config = yaml.safe_load(f)
            with open(self.adaptive_config_path, 'r', encoding='utf-8') as f:
                self._temp_adaptive_config = yaml.safe_load(f)
        except Exception as e:
            logging.error(f"[N-W10] 사전 config 로드 실패: {e}")
            self._temp_config = {}
            self._temp_adaptive_config = {}

        for filter_name, pass_rate in filters_to_adjust:
            logging.warning(f"[FIX] [{filter_name}] 필터 조정 시작")
            ...
            if self._adjust_filter(filter_name, pass_rate, optimized_criteria.get(filter_name, {})):
                ...
                logging.info(f"     [O] {filter_name} 필터 조정 완료")
            else:
                logging.error(f"     [X] {filter_name} 필터 조정 실패")
        ...
        if adjustments:
            ...
            # [O] FIX: 싱글톤 FilterManager 초기화하여 다음 사이클에서 새 설정 적용
            try:
                from .filter_manager import FilterManager
                FilterManager.reset_instance()
                logging.info("[O] FilterManager 싱글톤 초기화 완료 - 다음 사이클에서 새 설정이 적용됩니다.")
            except Exception as e:
                logging.warning(f"FilterManager 초기화 실패: {e}")

            # [O] NEW: 필터 조건 스냅샷 저장 (롤백 지원)
            ...
            # 조정 결과 요약
            logging.info("\n" + "="*50)
            logging.info("[STAT] 필터 자동 조정 완료 요약")
            logging.info("="*50)
            logging.info(f"[TARGET] 조정된 필터 수: {applied_count}개")
    ...
    def _adjust_filter(self, filter_name: str, current_pass_rate: float, optimization_hints: Dict) -> bool:
        ...
        try:
            ...
            # [FIX N-W10] 누적 config 사용 (사전 로드된 _temp_config/_temp_adaptive_config 재사용)
            # 루프 외부에서 이미 로드했으면 그것을 사용, 없으면 파일에서 로드
            ...
            if hasattr(self, '_temp_adaptive_config') and self._temp_adaptive_config:
                adaptive_config = self._temp_adaptive_config
            else:
                with open(self.adaptive_config_path, 'r', encoding='utf-8') as f:
            ...
            if filter_name == 'match':
                ...
                if 'dynamic_criteria' in adaptive_config and 'match' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - match.max_match: {old_max_match} → {new_max_match}")
                else:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - match 설정 추가: max_match=4")
                ...
                if 'filters' in config and 'criteria' in config['filters']:
                    if 'multiple' in config['filters']['criteria']:
                        ...
                        logging.info(f"       [NOTE] config.yaml - multiple.global_threshold: {old_threshold} → {new_threshold:.2f}")
                ...
                if 'dynamic_criteria' in adaptive_config and 'multiple' in adaptive_config['dynamic_criteria']:
                    ...
                    for key in adaptive_config['dynamic_criteria']['multiple']:
                        ...
                        if isinstance(current_range, list) and len(current_range) == 2:
                            ...
                            logging.info(f"       [NOTE] adaptive_config.yaml - multiple.{key}: {current_range} → {adaptive_config['dynamic_criteria']['multiple'][key]}")
                ...
                if 'filters' in config and 'criteria' in config['filters']:
                    if 'consecutive' in config['filters']['criteria']:
                        ...
                        logging.info(f"       [NOTE] config.yaml - consecutive.max_consecutive: {old_max} → {new_max}")
                ...
                if 'dynamic_criteria' in adaptive_config and 'consecutive' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - consecutive.max_consecutive: {old_max} → {new_max}")
                ...
                if 'filters' in config and 'criteria' in config['filters']:
                    if 'sum_range' in config['filters']['criteria']:
                        ...
                        logging.info(f"       [NOTE] config.yaml - sum_range: [{old_min}, {old_max}] → [{new_min}, {new_max}]")
                ...
                if 'dynamic_criteria' in adaptive_config and 'sum_range' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - sum_range: [{old_min}, {old_max}] → [{new_min}, {new_max}]")
                    ...
                    for step_type in ['all_steps', 'partial_steps', 'four_steps', 'three_steps']:
                        if step_type in fixed_step_config:
                            ...
                            if new_req != old_req:
                                ...
                                logging.info(f"       [NOTE] adaptive_config.yaml - fixed_step.{step_type}.required_matches: {old_req} → {new_req}")
                                adjusted = True
                    if not adjusted:
                        logging.info(f"       [O] fixed_step 필터: 이미 최소 매칭 수에 도달 (required_matches=2)")
                        ...
                        if section in adaptive_config['dynamic_criteria']['ten_section']:
                            ...
                            if isinstance(excluded_list, list) and len(excluded_list) > 0:
                                ...
                                if 6 in excluded_list:
                                    ...
                                    logging.info(f"       [NOTE] adaptive_config.yaml - ten_section.{section}: {old_excluded} → {excluded_list} (6개 허용)")
                                elif 0 in excluded_list:
                                    excluded_list.remove(0)
                                    logging.info(f"       [NOTE] adaptive_config.yaml - ten_section.{section}: {old_excluded} → {excluded_list} (0개 허용)")
                                else:
                                    # 이미 모두 허용 상태
                                    logging.info(f"       [O] ten_section.{section}: 이미 모든 개수 허용됨 (최적 상태)")

                                adaptive_config['dynamic_criteria']['ten_section'][section] = excluded_list
                            else:
                                logging.info(f"       [O] ten_section.{section}: 이미 빈 제외 리스트 (최적 상태)")
                ...
                if 'dynamic_criteria' in adaptive_config and 'odd_even' in adaptive_config['dynamic_criteria']:
                    ...
                    # 0과 6만 제외 (이미 최선)
                    logging.info(f"       [NOTE] odd_even 필터는 이미 최적 상태 (제외: {old_excluded})")
                ...
                if 'dynamic_criteria' in adaptive_config and 'max_gap' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - max_gap.max_allowed_gap: {old_max_gap} → {new_max_gap}")
                ...
                if 'dynamic_criteria' in adaptive_config and 'average' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - average: [{old_min}, {old_max}] → [{new_min}, {new_max}]")
                ...
                if 'dynamic_criteria' in adaptive_config and 'ac_value' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - ac_value.min_ac: {old_min_ac} → {new_min_ac}")
                else:
                    # dynamic_criteria에 ac_value가 없으면 추가
                    if 'dynamic_criteria' not in adaptive_config:
                        adaptive_config['dynamic_criteria'] = {}
                    adaptive_config['dynamic_criteria']['ac_value'] = {'min_ac': 6, 'max_ac': 10}
                    logging.info(f"       [NOTE] adaptive_config.yaml - ac_value 설정 추가: min_ac=6, max_ac=10")
                ...
                if 'dynamic_criteria' in adaptive_config and 'balanced_quadrant' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - balanced_quadrant.max_per_quadrant: {old_max} → {new_max}")
                else:
                    if 'dynamic_criteria' not in adaptive_config:
                        adaptive_config['dynamic_criteria'] = {}
                    adaptive_config['dynamic_criteria']['balanced_quadrant'] = {'max_per_quadrant': 4}
                    logging.info(f"       [NOTE] adaptive_config.yaml - balanced_quadrant 설정 추가: max_per_quadrant=4")
                ...
                if 'dynamic_criteria' in adaptive_config and 'dispersion' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - dispersion: [{old_min}, {old_max}] → [{new_min}, {new_max}]")
                else:
                    if 'dynamic_criteria' not in adaptive_config:
                        adaptive_config['dynamic_criteria'] = {}
                    adaptive_config['dynamic_criteria']['dispersion'] = {'min_std_dev': 2.0, 'max_std_dev': 20.0}
                    logging.info(f"       [NOTE] adaptive_config.yaml - dispersion 설정 추가")
                ...
                if 'dynamic_criteria' in adaptive_config and 'digit_sum' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - digit_sum: [{old_min}, {old_max}] → [{new_min}, {new_max}]")
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - digit_sum_range: [{old_range_min}, {old_range_max}] → [{new_range_min}, {new_range_max}]")
                else:
                    if 'dynamic_criteria' not in adaptive_config:
                        adaptive_config['dynamic_criteria'] = {}
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - digit_sum 설정 추가 (range 포함)")
                ...
                if 'dynamic_criteria' in adaptive_config and 'outlier_detection' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - outlier_detection: max_outliers {old_max_outliers} → {new_max_outliers}, iqr {old_iqr} → {new_iqr}")
                else:
                    if 'dynamic_criteria' not in adaptive_config:
                        adaptive_config['dynamic_criteria'] = {}
                    adaptive_config['dynamic_criteria']['outlier_detection'] = {'max_outliers': 2, 'iqr_multiplier': 1.5}
                    logging.info(f"       [NOTE] adaptive_config.yaml - outlier_detection 설정 추가")
                ...
                if 'dynamic_criteria' in adaptive_config and 'last_digit' in adaptive_config['dynamic_criteria']:
                    ...
                    logging.info(f"       [NOTE] adaptive_config.yaml - last_digit.min_same_last_digits: {old_min} → {new_min}")
                ...
                if 'dynamic_criteria' in adaptive_config and 'arithmetic' in adaptive_config['dynamic_criteria']:
                    ...
                    if 5 in old_exclude:
                        ...
                        logging.info(f"       [NOTE] adaptive_config.yaml - arithmetic.exclude_lengths: {old_exclude} → {new_exclude}")
                    else:
                        logging.info(f"       [O] arithmetic 필터: 이미 최적 상태 (exclude_lengths: {old_exclude})")
                ...
                if 'dynamic_criteria' in adaptive_config and 'geometric' in adaptive_config['dynamic_criteria']:
                    ...
                    if 4 in old_exclude:
                        ...
                        logging.info(f"       [NOTE] adaptive_config.yaml - geometric.exclude_lengths: {old_exclude} → {new_exclude}")
                    elif 5 in old_exclude:
                        new_exclude = [l for l in old_exclude if l != 5]
                        adaptive_config['dynamic_criteria']['geometric']['exclude_lengths'] = new_exclude
                        logging.info(f"       [NOTE] adaptive_config.yaml - geometric.exclude_lengths: {old_exclude} → {new_exclude}")
                    else:
                        logging.info(f"       [O] geometric 필터: 이미 최적 상태 (exclude_lengths: {old_exclude})")

            else:
                # [v5 FIX #16] 필터 통째 비활성화 fallback 제거 (전략 붕괴 방지).
                # 근거(53에이전트 감사): 통과율을 올리려고 필터를 끄면 표면 통과율은 95%로 보이나
                # 실제로는 815만->30만 축소 전략(핵심 가치)이 붕괴 - 통계적 극단 조합이 그대로 통과.
                # _enforce_filter_bounds 가드도 우회됨. 조정 핸들러가 없는 필터는 건드리지 않고 유지.
                logging.info(f"       [SKIP] {filter_name} 필터: 전용 조정 로직 없음 - 변경하지 않고 유지")
                logging.info(f"       [GUARD] 필터 비활성화 금지(핵심 전략 보호): 통과율은 threshold/criteria로만 조정")
    ...
    def _save_configs(self):
        ...
        if hasattr(self, '_temp_adaptive_config') and self._temp_adaptive_config:
            # [O] FIX: 기존 YAML 로드하여 critical fields 보존
            try:
                with open(self.adaptive_config_path, 'r', encoding='utf-8') as f:
            ...
            # [O] FIX: 전역 임계값 보존 (Optuna 최적화 값 보호)
            ...
            # [O] FIX: dynamic_criteria만 업데이트 (필터 기준값만 변경)
            ...
            if hasattr(self, 'filter_manager') and self.filter_manager:
                try:
                    ...
                    for filter_name, filter_instance in self.filter_manager.filters.items():
                        ...
                        if new_criteria:
                            ...
                            if filter_name == 'match' and 'max_match' in new_criteria:
                                ...
                                if old_max != new_max:
                                    logging.info(f"   [O] [{filter_name}] 필터 기준 실시간 적용: max_match {old_max} → {new_max}")
                                    updated_count += 1
                    ...
                    if updated_count > 0:
                        logging.info(f"   [O] 총 {updated_count}개 필터의 기준이 실시간으로 업데이트되었습니다.")
                        logging.info(f"   [SKIP]  프로그램 재시작 없이 즉시 적용됩니다!")
                except Exception as e:
                    logging.error(f"   [WARN] 필터 기준 실시간 업데이트 실패: {e}")
    ...
    def _save_filter_snapshot(self, validation_results):
        ...
        try:
            ...
            if last_id:
                ...
                logging.info(f"[O] 필터 조건 스냅샷 저장 완료 (Performance ID: {last_id})")
