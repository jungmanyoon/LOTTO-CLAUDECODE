...
from ..utils.constants import SystemConstants  # Phase 2.10: 시스템 상수 임포트
import time
import psutil
import numpy as np
import threading
...
class FilterManager:
    ...
    필터 종류:
        - 기본 필터: odd_even, sum_range, consecutive, max_gap
        - 패턴 필터: match, fixed_step, section, ten_section
        - 수학 필터: average, prime_composite, digit_sum, dispersion
        - 시퀀스 필터: arithmetic_sequence, geometric_sequence
        - 기타: last_digit, multiple, ac_value
    ...
    Thread Safety:
        ...
        - 캐시 접근에 threading.RLock 사용
        - Observer 패턴으로 설정 변경 자동 전파

    Observer 패턴:
        ThresholdManager의 임계값 변경 시 _on_config_change() 자동 호출되어
        필터 criteria가 재계산됩니다.
    ...
    See Also:
        - BaseFilter: 모든 필터의 추상 기반 클래스
        - ThresholdManager: 임계값 중앙 관리
        - FilterPerformanceTracker: 필터 성능 추적
    """
    # 클래스 변수 - 싱글톤 패턴 구현
    _instance = None
    ...
    def __init__(self, db_manager):
        ...
        with FilterManager._lock:
            ...
            # 배치 크기와 메모리 제한 설정 (Phase 2.10: 상수 사용)
            ...
            # 증분식 필터링 캐시 (Thread-Safe)
            self.incremental_cache = {}
            self._cache_lock = threading.RLock()  # 캐시 Thread-Safety를 위한 락
    ...
    def _on_config_change(self, param: str, old_value: Any, new_value: Any):
        ...
        if param in ("threshold", "global_probability_threshold"):
            ...
            # 🔥 FIX: 필터 criteria 재계산 및 업데이트
            self._recalculate_filter_criteria(float(new_value))
    ...
    def _recalculate_filter_criteria(self, new_threshold: float):
        ...
        """
        try:
            logging.info(f"[FilterManager] 임계값 {new_threshold}% 기준으로 필터 criteria 재계산 시작")

            # ConfigManager에서 새로운 criteria 가져오기
            # (adaptive_filter_config.yaml의 dynamic_criteria는
            #  AdaptiveProbabilityFilter.generate_dynamic_criteria()에서 재계산됨)
            updated_count = 0
            ...
            logging.info(f"[FilterManager] 필터 criteria 재계산 완료: {updated_count}개 필터 업데이트됨")

        except Exception as e:
            logging.error(f"[FilterManager] 필터 criteria 재계산 중 오류: {e}")
    ...
    def _auto_register_filters(self):
        ...
        filter_categories = {
            ...
            "신규 필터": ["multiple", "ten_section", "arithmetic_sequence", "geometric_sequence",
                         "prime_composite", "digit_sum", "dispersion", "ac_value"],
    ...
    def apply_filters_streaming(self, latest_round: int) -> bool:
        ...
        try:
            ...
            # 기본 통계 초기화
            # 기본 통계 초기화 (제거됨 - stream_processor 결과 사용)
            # stats = {
    ...
    def _log_filter_summary(self):
        ...
        try:
            ...
            filter_groups = {
                ...
                "통계 필터": ["outlier_detection", "balanced_quadrant", "ac_value"],
        ...
        try:
            ...
            with self._cache_lock:
                if cache_key in self.incremental_cache:
                    ...
                    # 전체 조합에서 시작
                    last_filter_index = 0
                    remaining_combinations = None

            if remaining_combinations is None:
            ...
            with tqdm(
                ...
                for i, (filter_name, filter_instance) in enumerate(sorted_filters[last_filter_index:], last_filter_index):
                    ...
                    # Thread-Safe 캐시 업데이트 (중단 후 재개를 위해)
                    with self._cache_lock:
            ...
            # Thread-Safe 캐시 제거 (성공적으로 완료된 경우)
            with self._cache_lock:
                if cache_key in self.incremental_cache:
